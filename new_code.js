...
function checkOnlineWarning() {
  const onlineWarning = document.getElementById('online-warning');
  const selectFieldValue = selectField3.value;
  const inPersonRadio = document.getElementById('In-Person');

  if (inPersonRadio.checked && (selectFieldValue === 'QCE' || selectFieldValue === 'SACE' || selectFieldValue === 'WACE')) {
...
// New event listener for "In-Person" radio button
document.getElementById('In-Person').addEventListener('change', function() {
  checkOnlineWarning(); // Add this line
});

// Event listener for selectField3
